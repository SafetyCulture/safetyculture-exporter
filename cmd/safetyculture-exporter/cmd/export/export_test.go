...
func TestNewConfigurationManagerFromFile_should_apply_the_viper_defaults(t *testing.T) {
	...
	viperConfig.Set("export.schedule_occurrences.start_date", "2022-01-20")
	viperConfig.Set("export.schedule_occurrences.end_date", "2022-01-21")
	...
	occurrencesStartDate, err := time.Parse("2006-01-02", "2022-01-20")
	assert.Nil(t, err)
	assert.Equal(t, occurrencesStartDate, cm.Configuration.Export.ScheduleOccurrences.StartDate.Time)
	occurrencesEndDate, err := time.Parse("2006-01-02", "2022-01-21")
	assert.Nil(t, err)
	assert.Equal(t, occurrencesEndDate, cm.Configuration.Export.ScheduleOccurrences.EndDate.Time)

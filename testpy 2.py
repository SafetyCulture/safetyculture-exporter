...
class SafetyCulture:
    ...
    def authenticated_request_get(self, url):
        if self.proxy is not None and self.certs is not None:
            return requests.get(url, headers=self.custom_http_headers, proxies=self.proxy, verify=self.certs)
        elif self.certs is None and self.proxy is not None:
            return requests.get(url, headers=self.custom_http_headers, proxies=self.proxy)
        elif self.certs is not None and self.proxy is None:
            return requests.get(url, headers=self.custom_http_headers, verify=self.certs)
        else:
    ...
    def authenticated_request_post(self, url, data):
        ...
        if self.proxy is not None and self.certs is not None:
            response = requests.post(url, data, headers=self.custom_http_headers, proxies=self.proxy, verify=self.certs)
        elif self.certs is None and self.proxy is not None:
            response = requests.post(url, data, headers=self.custom_http_headers, proxies=self.proxy)
        elif self.certs is not None and self.proxy is None:
            response = requests.post(url, data, headers=self.custom_http_headers, verify=self.certs)
        else:
    ...
    def authenticated_request_put(self, url, data):
        self.custom_http_headers['content-type'] = 'application/json'
        if self.proxy is not None and self.certs is not None:
            response = requests.put(url, data, headers=self.custom_http_headers, proxies=self.proxy, verify=self.certs)
        elif self.certs is None and self.proxy is not None:
            response = requests.put(url, data, headers=self.custom_http_headers, proxies=self.proxy)
        elif self.certs is not None and self.proxy is None:
            response = requests.put(url, data, headers=self.custom_http_headers, verify=self.certs)
        else:
    ...
    def authenticated_request_delete(self, url):
        if self.proxy is not None and self.certs is not None:
            return requests.delete(url, headers=self.custom_http_headers, proxies=self.proxy, verify=self.certs)
        elif self.certs is None and self.proxy is not None:
            return requests.delete(url, headers=self.custom_http_headers, proxies=self.proxy)
        elif self.certs is not None and self.proxy is None:
            return requests.delete(url, headers=self.custom_http_headers, verify=self.certs)
        else:
